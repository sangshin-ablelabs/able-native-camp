...
function getSessionCachePath() {
  return path.join(os.homedir(), ".config", "ainc", "session-cache.json");
}
...
function getQueuePath() {
  return path.join(os.homedir(), ".config", "ainc", "queue.jsonl");
}
...
function selfUpdate(apiUrl) {
  try {
    const configDir = path.join(os.homedir(), ".config", "ainc");
          ...
          if (fs.existsSync(settingsPath)) {
            ...
            if (settings.hooks && settings.hooks.Stop) {
              ...
              for (const entry of settings.hooks.Stop) {
                const isAinc = entry.hooks && entry.hooks.some((h) => h.command && h.command.includes("ainc/report-usage"));
                if (isAinc && entry.matcher !== ".*") {
                  entry.matcher = ".*";
                  fixed = true;
                }
              }
